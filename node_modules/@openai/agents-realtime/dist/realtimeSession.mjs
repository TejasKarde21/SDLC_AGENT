...
import { getSafeErrorType, hasDynamicFunctionToolApprovalPolicy, hasInspectableFunctionToolArguments, logToolActionError, } from '@openai/agents-core/utils/internal';
...
export class RealtimeSession extends RuntimeEventEmitter {
    ...
    async #handleHandoff(toolCall, handoff, sourceAgent) {
    ...
    async #handleFunctionCall(toolCall, dispatchSnapshot) {
        ...
        if (functionTool) {
            await this.#handleFunctionToolCall(toolCall, functionTool, filteredSnapshot.agent, filteredSnapshot);
            return;
        }
        const possibleHandoff = handoffMap.get(toolCall.name);
        if (possibleHandoff) {
            await this.#handleHandoff(toolCall, possibleHandoff, filteredSnapshot.agent);
            return;
        }
