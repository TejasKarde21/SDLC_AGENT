...
exports.CompactionItemValidationError = void 0;
exports.toAgentInputList = toAgentInputList;
exports.assertValidCompactionItems = assertValidCompactionItems;
...
exports.deduplicateAgentInputItemsPreferringLatest = deduplicateAgentInputItemsPreferringLatest;
...
exports.trimToLatestCompaction = trimToLatestCompaction;
...
class CompactionItemValidationError extends errors_1.UserError {
}
exports.CompactionItemValidationError = CompactionItemValidationError;
...
function assertValidCompactionItems(items) {
    for (const item of items) {
        if (item.type === 'compaction' &&
            !protocol.CompactionItem.safeParse(item).success) {
            throw new CompactionItemValidationError('Compaction item missing encrypted_content');
        }
    }
}
...
function getAgentInputItemDeduplicationKey(item) {
    ...
    if (itemType === 'tool_search_call' || itemType === 'tool_search_output') {
        const callId = (0, tooling_1.getToolSearchProviderCallId)(item);
        if (callId) {
            return JSON.stringify(['call', itemType, callId]);
        }
    }
    const callCorrelation = (0, toolResultCorrelation_1.getToolResultCorrelationForCall)(item);
    ...
    const resultCorrelation = (0, toolResultCorrelation_1.getToolResultCorrelationForResult)(item);
...
function isCausalPrecursorItem(item) {
    if (!item || typeof item !== 'object') {
        return false;
    }
    return (item.type === 'tool_search_call' ||
        (item.type === 'hosted_tool_call' &&
            (0, toolResultCorrelation_1.getToolResultCorrelationForResult)(item) === undefined) ||
        item.type === 'compaction' ||
        item.type === 'reasoning' ||
        (0, toolResultCorrelation_1.getToolResultCorrelationForCall)(item) !== undefined);
}
/**
 ...
 */
function deduplicateAgentInputItemsPreferringLatest(items) {
    const latestByKey = new Map();
    const anchorIndexByKey = new Map();
    ...
    for (const [index, item] of items.entries()) {
        ...
        if (!key) {
            ...
            deduplicated.push(latestByKey.get(key));
        }
    }
    return deduplicated;
}
...
function collectCompletedCallIdsByResultType(items) {
    ...
    for (const item of items) {
        ...
        if (!(0, toolResultCorrelation_1.isSimpleToolResultType)(type)) {
            continue;
        }
...
function collectProgramCallIds(items) {
...
function getProgramCallerId(item) {
    ...
    const candidate = caller;
    return candidate.type === 'program' && typeof candidate.callerId === 'string'
        ? candidate.callerId
        : undefined;
}
function getProgramOwnedCorrelationKey(programCallId, correlation) {
    return correlation
        ? JSON.stringify([programCallId, (0, toolResultCorrelation_1.getToolResultCorrelationKey)(correlation)])
        : undefined;
}
function collectProgramOwnedCallKeys(items) {
    ...
    for (const item of items) {
        ...
        const key = getProgramOwnedCorrelationKey(programCallId, (0, toolResultCorrelation_1.getToolResultCorrelationForCall)(item));
        if (key) {
            callKeys.add(key);
        }
    }
...
function hasRetainedProgramOwnedItem(items, programCallId, pruningIndexes) {
    const retainedCallKeys = new Set();
    const retainedResultKeys = new Set();
    for (const [index, item] of items.entries()) {
        if (getProgramCallerId(item) !== programCallId) {
            continue;
        }
        const result = (0, toolResultCorrelation_1.getToolResultCorrelationForResult)(item);
        if (!(pruningIndexes?.has(index) ?? false) &&
        ...
        const call = (0, toolResultCorrelation_1.getToolResultCorrelationForCall)(item);
        if (call) {
            retainedCallKeys.add((0, toolResultCorrelation_1.getToolResultCorrelationKey)(call));
        }
        if (result) {
            retainedResultKeys.add((0, toolResultCorrelation_1.getToolResultCorrelationKey)(result));
        }
    }
...
function dropOrphanToolCalls(items, options) {
    ...
    const activeProgramCallIds = new Set();
    const orphanProgramCallIds = new Set();
    for (const [index, item] of items.entries()) {
        ...
        if (completedByResultType.get('program_output')?.has(item.callId) ??
            false) {
            continue;
        }
        const hasRetainedOwnedItem = hasRetainedProgramOwnedItem(items, item.callId, pruningIndexes);
    ...
    const filtered = items.filter((item, index) => {
        ...
        if (programCallerId) {
            if (!programCallIds.has(programCallerId) ||
                orphanProgramCallIds.has(programCallerId)) {
                droppedIndexes.add(index);
                return false;
            }
            const resultKey = getProgramOwnedCorrelationKey(programCallerId, (0, toolResultCorrelation_1.getToolResultCorrelationForResult)(item));
        ...
        const resultType = (0, toolResultCorrelation_1.getSimpleToolResultTypeForCall)(type);
        if (!resultType) {
            return true;
        }
