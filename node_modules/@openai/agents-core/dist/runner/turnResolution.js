...
function resolveApprovalState(item, state, processedResponse) {
    ...
    const approval = toolNames
        .map((candidate) => state._context.isToolApproved({
        toolName: candidate,
        callId,
        functionTool: false,
...
async function resolveInvalidFinalOutput(args) {
    ...
    (0, errorHandlers_1.validateRunErrorHandlerFinalOutput)(args.agent, outputText);
 ...
 */
async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal) {
 ...
 */
async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, errorHandlers, signal) {
    ...
    const runFunctionTools = (executionSignal = signal, cancelSiblingCategories) => (0, toolExecution_1.executeFunctionToolCalls)(agent, processedResponse.functions, runner, state, toolErrorFormatter, agentToolParentRunConfig, executionSignal, cancelSiblingCategories);
    const runComputerActions = (executionSignal = signal, cancelSiblingCategories) => (0, toolExecution_1.executeComputerActions)(agent, processedResponse.computerActions, runner, state._context, undefined, toolErrorFormatter, executionSignal, cancelSiblingCategories);
    const [functionResults, computerResults] = processedResponse.functions.length > 0 &&
    ...
    if (processedResponse.handoffs.length > 0) {
        if (signal?.aborted) {
            ...
            return await (0, toolExecution_1.executeHandoffCalls)(agent, originalInput, preStepItems, newItems, newResponse, processedResponse.handoffs, runner, state._context, (0, invocationContext_1.getRunStateTurnSpanParent)(state) ?? state._currentAgentSpan);
        }
    }
    const completedStep = await maybeCompleteTurnFromToolResults({
        agent,
    ...
    if (!hasPendingToolsOrApprovals) {
        ...
        if (agent.outputType !== 'text' && potentialFinalOutput) {
            ...
            if (error) {
                ...
                const outputError = new errors_1.ModelBehaviorError(outputErrorMessage);
                const handledOutput = await resolveInvalidFinalOutput({
                    error: outputError,
                    errorHandlers,
                    agent,
