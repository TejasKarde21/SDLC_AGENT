...
import { getFunctionToolStateKey, getFunctionToolStateKeyForCall, getFunctionToolStateKeys, } from "../toolIdentity.mjs";
import { createRunErrorFinalOutputItem, formatRunErrorFinalOutput, resolveRunErrorHandler, validateRunErrorHandlerFinalOutput, } from "./errorHandlers.mjs";
 ...
 */
export async function resolveInterruptedTurn(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal) {
 ...
 */
export async function resolveTurnAfterModelResponse(agent, originalInput, originalPreStepItems, newResponse, processedResponse, runner, state, toolErrorFormatter, agentToolParentRunConfig, errorHandlers, signal) {
    ...
    const runFunctionTools = (executionSignal = signal, cancelSiblingCategories) => executeFunctionToolCalls(agent, processedResponse.functions, runner, state, toolErrorFormatter, agentToolParentRunConfig, executionSignal, cancelSiblingCategories);
    const runComputerActions = (executionSignal = signal, cancelSiblingCategories) => executeComputerActions(agent, processedResponse.computerActions, runner, state._context, undefined, toolErrorFormatter, executionSignal, cancelSiblingCategories);
    ...
    if (processedResponse.handoffs.length > 0) {
        if (signal?.aborted) {
            ...
            return await executeHandoffCalls(agent, originalInput, preStepItems, newItems, newResponse, processedResponse.handoffs, runner, state._context, getRunStateTurnSpanParent(state) ?? state._currentAgentSpan);
        }
    }
    const completedStep = await maybeCompleteTurnFromToolResults({
        agent,
