...
function getToolCallOutputItem(toolCall, output, options) {
    ...
    if (hasOutputSchema) {
        try {
            ...
            throw new errors_1.InvalidToolOutputError(`Function tool '${toolCall.name}' outputSchema requires a JSON-serializable output.`, undefined, error, { output });
        }
    }
    else {
...
async function executeFunctionToolCalls(agent, toolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, onFatalFailure) {
    ...
    try {
        ...
        throw new errors_1.ToolCallError(`Failed to run function tools: ${surfacedError}`, surfacedError, redactInvalidInputFailure || (0, toolInputError_1.isRedactedInvalidToolInputError)(e)
            ? undefined
            : state);
    }
}
...
function buildFunctionFailureResult(deps, toolRun, output) {
    return {
        ...
        tool: toolRun.tool,
        output,
        runItem: new items_1.RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, output, {
...
async function resolveFunctionFailureOutput(deps, toolRun, error, legacyOutput, redactionBoundary) {
    ...
    try {
        ...
        const validatedOutput = (0, tool_1.validateFunctionToolOutput)({
            tool: toolRun.tool,
            output,
            runContext: deps.state._context,
            details,
    ...
    return withRunStateToolFunctionSpan(deps, traceToolName, async (span) => {
        ...
        try {
            ...
            const functionResult = {
                ...
                tool: toolRun.tool,
                output: toolOutput,
                runItem: new items_1.RunToolCallOutputItem(rawItem, agent, toolOutput, customData, executionStatus),
            };
 ...
 */
async function executeHandoffCalls(agent, originalInput, preStepItems, newStepItems, newResponse, runHandoffs, runner, runContext, parent) {
