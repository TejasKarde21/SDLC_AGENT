...
import { buildAgentInputPool, deduplicateAgentInputItemsPreferringLatest, extractOutputItemsFromRunItems, getAgentInputItemKey, removeAgentInputFromPool, takeAgentInputFromPool, toAgentInputList, } from "./items.mjs";
import { getToolResultCorrelationForResult, getToolResultCorrelationKey, getUnresolvedToolResultCorrelationsForResponse, } from "./toolResultCorrelation.mjs";
...
export async function applyCallModelInputFilter(agent, callModelInputFilter, context, inputItems, systemInstructions) {
    ...
    if (!callModelInputFilter) {
        const normalizedInput = deduplicateAgentInputItemsPreferringLatest(clonedBaseInput);
        return {
    ...
    try {
        ...
        const normalizedBaseSources = deduplicateAgentInputItemsPreferringLatest(clonedBaseInput).map((item) => cloneMap.get(item) ?? item);
        ...
        const normalizedInput = deduplicateAgentInputItemsPreferringLatest(result.input);
        ...
        const sourceItems = normalizedInput.map((item, index) => {
            ...
            if (consumedExactClones.has(item)) {
                injectedExactCloneIndexes.add(index);
                return undefined;
            }
            consumedExactClones.add(item);
    ...
    primeFromState({ originalInput, generatedItems, modelResponses, }) {
        ...
        for (const [responseIndex, response] of modelResponses.entries()) {
            if (responseIndex < acknowledgingResponseIndex) {
                const unresolvedCorrelations = getUnresolvedToolResultCorrelationsForResponse(response.output);
        ...
        if (hasResponses) {
            for (const item of generatedItems) {
                ...
                const correlation = getToolResultCorrelationForResult(rawItem);
                if (correlation &&
