...
import { clearToolErrorState, InvalidToolOutputError, isToolTimeoutError, ToolCallError, UserError, } from "../errors.mjs";
...
import { FUNCTION_TOOL_PARSED_INPUT_CALLBACK, invokeFunctionTool, prepareFunctionToolInput, hasDynamicFunctionToolApprovalPolicy, hasInspectableFunctionToolArguments, setFunctionToolPreparedInput, validateFunctionToolOutput, resolveComputer, } from "../tool.mjs";
...
export function getToolCallOutputItem(toolCall, output, options) {
    ...
    if (hasOutputSchema) {
        try {
            ...
            throw new InvalidToolOutputError(`Function tool '${toolCall.name}' outputSchema requires a JSON-serializable output.`, undefined, error, { output });
        }
    }
    else {
...
export async function executeFunctionToolCalls(agent, toolRuns, runner, state, toolErrorFormatter, agentToolParentRunConfig, signal, onFatalFailure) {
    ...
    const executeToolRun = async (toolRun, executionSignal = signal) => {
        ...
        if (!parseResult.success) {
            if (parseResult.preparedInput) {
                ...
                try {
                    return await runApprovedFunctionTool(executionDeps, toolRun, parseResult.approvalArgs, parseResult.preparedInput, failure);
                }
                catch (error) {
                    if (executionSignal?.aborted &&
        ...
        try {
            return await runApprovedFunctionTool(executionDeps, toolRun, parseResult.approvalArgs, parseResult.preparedInput);
        }
        catch (error) {
            if (executionSignal?.aborted &&
                (error === executionSignal.reason || isAbortError(error))) {
    ...
    try {
        ...
        const surfacedError = e;
        throw new ToolCallError(`Failed to run function tools: ${surfacedError}`, surfacedError, redactInvalidInputFailure || isRedactedInvalidToolInputError(e)
            ? undefined
            : state);
    }
}
...
function buildFunctionFailureResult(deps, toolRun, output) {
    return {
        ...
        tool: toolRun.tool,
        output,
        runItem: new RunToolCallOutputItem(getToolCallOutputItem(toolRun.toolCall, output, {
...
async function runApprovedFunctionTool(deps, toolRun, approvalArgs, preparedInput, invalidInputFailure) {
    ...
    return withRunStateToolFunctionSpan(deps, traceToolName, async (span) => {
        ...
        try {
            ...
            const functionResult = {
                ...
                tool: toolRun.tool,
                output: toolOutput,
                runItem: new RunToolCallOutputItem(rawItem, agent, toolOutput, customData, executionStatus),
            };
...
async function resolveToolApproval(options) {
    const { runContext, toolName, callId, approvalItem, needsApproval, onApproval, isCancelled, } = options;
...
async function handleToolApprovalDecision(options) {
    const { runContext, toolName, callId, approvalItem, needsApproval, onApproval, buildRejectionItem, isCancelled, buildCancellationItem, } = options;
 ...
 */
export async function executeHandoffCalls(agent, originalInput, preStepItems, newStepItems, newResponse, runHandoffs, runner, runContext, parent) {
...
export async function checkForFinalOutputFromTools(agent, toolResults, state, additionalInterruptions = []) {
    ...
    if (typeof toolUseBehavior === 'function') {
        return toolUseBehavior(state._context, finalizationResults);
    }
