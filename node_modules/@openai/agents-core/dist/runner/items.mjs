...
import { getToolSearchProviderCallId } from "../tooling.mjs";
import * as protocol from "../types/protocol.mjs";
import { serializeBinary } from "../utils/binary.mjs";
import { getToolResultCorrelationForCall, getToolResultCorrelationForResult, getToolResultCorrelationKey, getSimpleToolResultTypeForCall, isSimpleToolResultType, } from "./toolResultCorrelation.mjs";
...
export function assertValidCompactionItems(items) {
 ...
 */
export function deduplicateAgentInputItemsPreferringLatest(items) {
...
function collectProgramOwnedCallKeys(items) {
    ...
    for (const item of items) {
        ...
        const key = getProgramOwnedCorrelationKey(programCallId, getToolResultCorrelationForCall(item));
        if (key) {
            callKeys.add(key);
        }
    }
...
export function dropOrphanToolCalls(items, options) {
    ...
    const filtered = items.filter((item, index) => {
        ...
        if (programCallerId) {
            ...
            const resultKey = getProgramOwnedCorrelationKey(programCallerId, getToolResultCorrelationForResult(item));
