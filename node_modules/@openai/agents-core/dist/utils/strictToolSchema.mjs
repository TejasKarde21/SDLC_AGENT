...
export function toOpenAIStrictToolSchema(schema) {
    return ensureStrictSchemaEntry(structuredClone(schema), true);
}
function ensureStrictSchemaEntry(entry, isRoot = false) {
    ...
    const properties = isRecord(record.properties)
        ? record.properties
        : undefined;
    const hasObjectKeywords = properties !== undefined || 'additionalProperties' in record;
    if (record.type === undefined &&
        hasObjectKeywords &&
        record.additionalProperties !== false) {
        throw new UserError('Cannot convert a typeless open JSON schema to strict mode. Set `type: "object"` with `additionalProperties: false`, or disable strict mode.');
    }
...
export function stripStrictNullsForJsonSchema(schema, value, optionalProperty = false) {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
...
export function stripStrictNullsForZodSchema(schema, value) {
...
//# sourceMappingURL=strictToolSchema.mjs.map