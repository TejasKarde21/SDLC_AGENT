...
import { AgentsError, ModelBehaviorError, OutputGuardrailTripwireTriggered, UserError, } from "./errors.mjs";
...
import { attachRunStateToError, prepareRunErrorFinalOutput, } from "./runner/errorHandlers.mjs";
...
export class Runner extends RunHooks {
    ...
    async run(agent, input, options = {
        ...
        return withTrace(this.config.workflowName ?? 'Agent workflow', async (trace) => executeInInvocationTrace(trace), {
            ...
            const finalizeCurrentOutput = async () => {
                ...
                try {
                    ...
                    if (error instanceof OutputGuardrailTripwireTriggered &&
                        persistResult) {
                        try {
                            await persistResult(new RunResult(state), {
                                outputBlocked: true,
                            });
                        }
            ...
            try {
                while (true) {
                    ...
                    if (state._currentStep.type === 'next_step_run_again') {
                        ...
                        const turnResult = await resolveTurnAfterModelResponse(state._currentAgent, state._originalInput, state._generatedItems, state._lastTurnResponse, state._lastProcessedResponse, this, state, toolErrorFormatter, agentToolParentRunConfig, options.errorHandlers, options.signal);
                        applyTurnResult({
                            state,
                            turnResult,
                            agent: state._currentAgent,
    ...
    async #runStreamLoop(result, startingAgent, sandboxRuntime, options, isResumedState, ensureStreamInputPersisted, sessionTurnInputUpdate, sessionInputUpdate, getStreamInputForPersistence, preserveTurnPersistenceOnResume, provisionalSessionHistoryTransactionSessionId, provisionalSessionHistoryTransactionInputItems, sandboxMemoryRunContext, taskSpan, invocationSpanParent) {
        ...
        const saveStreamResultWithCompactionOwnership = async (overrideOptions) => {
            ...
            streamInputPersisted = true;
            await saveStreamResultToSession(options.session, result, persistenceOptions, sessionInputItems);
        };
        ...
        try {
            while (true) {
                ...
                if (result.state._currentStep.type === 'next_step_run_again') {
                    ...
                    const turnResult = await resolveTurnAfterModelResponse(currentAgent, result.state._originalInput, result.state._generatedItems, result.state._lastTurnResponse, result.state._lastProcessedResponse, this, result.state, toolErrorFormatter, agentToolParentRunConfig, options.errorHandlers, options.signal);
                    applyTurnResult({
                        state: result.state,
                        turnResult,
                        agent: currentAgent,
    ...
    async #prepareModelCall(state, executionAgent, options, artifacts, turnInput, serverConversationTracker, sessionInputUpdate) {
        ...
        modelSettings = adjustModelSettingsForNonGPT5RunnerModel(explicitlyModelSet, agentModelSettings ?? implicitModelSettings ?? {}, model, modelSettings, resolvedModelName);
