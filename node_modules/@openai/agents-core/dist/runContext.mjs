...
import { getFunctionToolLookupKey, getFunctionToolLegacyStateKeyFromStateKey, getFunctionToolStateKeyForCall, } from "./toolIdentity.mjs";
...
export class RunContext {
    ...
    _validateFunctionApprovalOwners(approvals, agentsByIdentity) {
        ...
        for (const { agentIdentity } of approvals) {
            if (seenAgentIdentities.has(agentIdentity) ||
                ...
                throw new UserError('RunState contains invalid function approval ownership for the current agent graph.');
            }
    ...
    #serialize(agentIdentityKeys) {
        ...
        if (!agentIdentityKeys) {
            ...
            for (const approvalsByTool of this.#functionApprovalState.approvalsByAgent.values()) {
                for (const [toolName, incoming] of approvalsByTool) {
                    const publicToolName = getFunctionToolLegacyStateKeyFromStateKey(toolName) ?? toolName;
    ...
    #getLegacyFunctionApprovalEntries(toolName) {
        const legacyToolName = getFunctionToolLegacyStateKeyFromStateKey(toolName) ?? toolName;
