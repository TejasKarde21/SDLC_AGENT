...
import { logger as globalLogger, logToolActionWarning, } from "./logger.mjs";
import { getMcpServerExternalName } from "./mcpLogging.mjs";
import { maybeExtractToolOutputCustomData } from "./utils/customData.mjs";
import { BaseMCPServerSSE, BaseMCPServerStdio, BaseMCPServerStreamableHttp, } from "./mcpShared.mjs";
import { cachedMcpToolKeysByServer as _cachedToolKeysByServer, cachedMcpTools as _cachedTools, } from "./mcpToolCache.mjs";
import { getToolCallParentSpanFromDetails } from "./agentToolRunConfig.mjs";
export { BaseMCPServerSSE, BaseMCPServerStdio, BaseMCPServerStreamableHttp, DEFAULT_SSE_MCP_CLIENT_LOGGER_NAME, DEFAULT_STDIO_MCP_CLIENT_LOGGER_NAME, DEFAULT_STREAMABLE_HTTP_MCP_CLIENT_LOGGER_NAME, MCPTool, attachCallToolResultMetadata, } from "./mcpShared.mjs";
export { invalidateServerToolsCache } from "./mcpToolCache.mjs";
            ...
            for (const tool of fetchedMcpTools) {
                ...
                if (filter) {
                    if (typeof filter === 'function') {
                        ...
                        if (!filtered) {
                            logMcpToolFilterDebug(() => `MCP Tool (server: ${getMcpServerExternalName(server.name)}, tool: ${tool.name}) is blocked by the callable filter.`);
                            continue;
                        }
                    }
                    else {
                        ...
                        if (allowedToolNames.length > 0 || blockedToolNames.length > 0) {
                            ...
                            if (!allowed || blocked) {
                                if (blocked) {
                                    logMcpToolFilterDebug(() => `MCP Tool (server: ${getMcpServerExternalName(server.name)}, tool: ${tool.name}) is blocked by the static filter.`);
                                }
                                else if (!allowed) {
                                    logMcpToolFilterDebug(() => `MCP Tool (server: ${getMcpServerExternalName(server.name)}, tool: ${tool.name}) is not allowed by the static filter.`);
                                }
                                continue;
                            }
                        }
    ...
    for (const { server, serverIndex, mcpTools } of serverToolBatches) {
        ...
        mcpTools.forEach((mcpTool, toolIndex) => {
            ...
            if (previousRawServerName !== undefined &&
                ...
                throw new UserError(`MCP server names are indistinguishable after URL redaction for tool '${mcpTool.name}': '${serverName}'. Configure unique safe server names when includeServerInToolNames is enabled.`);
            }
...
export function mcpToFunctionTool(mcpTool, server, convertSchemasToStrict, options = {}) {
    ...
    async function invoke(input, runContext, details) {
        ...
        const currentSpan = getToolCallParentSpanFromDetails(details) ?? getCurrentSpan();
        if (currentSpan) {
    ...
    if (convertSchemasToStrict || schema.additionalProperties === true) {
        try {
            ...
            logToolActionWarning(globalLogger, 'Error converting MCP schema to strict mode:', e);
        }
    }
