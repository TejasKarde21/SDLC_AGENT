...
import { combineAbortSignals, isAbortError, isSiblingCancellationSignal, } from "./utils/abortSignals.mjs";
import { InvalidToolOutputError, ToolTimeoutError, UserError } from "./errors.mjs";
import { createInvalidToolInputDisposition, createInvalidToolInputFailure, getInvalidToolInputFailure, isRedactedInvalidToolInputError, refreshInvalidToolInputFailure, } from "./toolInputError.mjs";
...
import { FUNCTION_TOOL_NAMESPACE, FUNCTION_TOOL_NAMESPACE_DESCRIPTION, getFunctionToolLookupKeyForTool, getFunctionToolQualifiedName, } from "./toolIdentity.mjs";
import { resolveToolInputGuardrails, resolveToolOutputGuardrails, } from "./toolGuardrail.mjs";
import { normalizeToolAllowedCallers, } from "./utils/toolCallers.mjs";
...
/** @internal */
export function prepareFunctionToolInput(tool, input) {
...
/** @internal */
export function setFunctionToolPreparedInput(details, preparedInput) {
...
export function hasDynamicFunctionToolApprovalPolicy(tool) {
    return !STATIC_FUNCTION_TOOL_APPROVAL_POLICIES.has(tool.needsApproval);
}
export function hasInspectableFunctionToolArguments(value) {
...
export async function disposeResolvedComputers({ runContext, }) {
    ...
    for (const dispose of disposers) {
        try {
            ...
            logToolActionWarning(logger, 'Failed to dispose computer for run context:', error);
        }
    }
}
...
export function shellTool(options) {
    ...
    const allowedCallers = normalizeToolAllowedCallers(options.allowedCallers, options.name ?? 'shell');
    const needsApproval = typeof options.needsApproval === 'function'
        ? options.needsApproval
        : async () => typeof options.needsApproval === 'boolean'
            ? options.needsApproval
...
export function applyPatchTool(options) {
    ...
    const allowedCallers = normalizeToolAllowedCallers(options.allowedCallers, name);
    const needsApproval = typeof options.needsApproval === 'function'
        ? options.needsApproval
        : async () => typeof options.needsApproval === 'boolean'
            ? options.needsApproval
...
export function hostedMcpTool(options) {
    const allowedCallers = normalizeToolAllowedCallers(options.allowedCallers, options.serverLabel);
...
/** @internal */
export function getToolSearchRuntimeRoutingKey(tool) {
    if (tool.type === 'function') {
 ...
 * output schema.
 */
export function validateFunctionToolOutput(args) {
...
export function tool(options) {
    ...
    if (hasOutputSchema &&
        ...
        throw new UserError(`Function tool '${name}' with an output schema requires timeoutErrorFunction when timeoutBehavior is 'error_as_result'.`);
    }
    const allowedCallers = normalizeToolAllowedCallers(options.allowedCallers, name);
    ...
    function validateOutput(output, runContext, details) {
        ...
        try {
            ...
            throw new InvalidToolOutputError(`Invalid output for function tool '${name}'.`, undefined, error, { runContext, output, details });
        }
    }
