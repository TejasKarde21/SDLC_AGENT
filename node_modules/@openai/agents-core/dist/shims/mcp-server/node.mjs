...
import { BaseMCPServerStdio, BaseMCPServerStreamableHttp, BaseMCPServerSSE, attachCallToolResultMetadata, } from "../../mcpShared.mjs";
import { invalidateServerToolsCache } from "../../mcpToolCache.mjs";
import logger, { getSafeErrorType, logToolActionError, logToolActionWarning, } from "../../logger.mjs";
import { sanitizeMcpTransportError } from "../../mcpLogging.mjs";
    ...
    async close() {
        ...
        if (transport && typeof transport.terminateSession === 'function') {
            try {
                ...
                logToolActionWarning(this.logger, 'Failed to terminate MCP session:', error);
            }
        }
        if (transport) {
...
export class NodeMCPServerSSE extends BaseMCPServerSSE {
    ...
    async connect() {
        ...
        try {
            ...
            const error = sanitizeMcpTransportError(e, this.params.url, 'SSE connect');
...
export class NodeMCPServerStreamableHttp extends BaseMCPServerStreamableHttp {
    ...
    async connect() {
        ...
        try {
            ...
            const error = sanitizeMcpTransportError(e, this.params.url, 'streamable HTTP connect');
            logToolActionError(this.logger, 'Error initializing MCP server:', error);
            throw error;
        }
        this.debugLog(() => `Connected to MCP server: ${this._name}`);
    ...
    async callToolResult(toolName, args, meta, options) {
        ...
        try {
            ...
            if (recoveryStrategy === 'none') {
                throw sanitizeMcpTransportError(error, this.params.url, 'streamable HTTP tool call');
            }
            ...
            try {
                ...
                throw sanitizeMcpTransportError(attachCause(retryError, error), this.params.url, 'streamable HTTP tool call retry');
            }
        }
        this.debugLog(() => `Called tool ${toolName} (args: ${JSON.stringify(args)}, result: ${JSON.stringify(result)})`);
        return result;
