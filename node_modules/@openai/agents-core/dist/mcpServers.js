...
function getMcpServerLogLabel(server) {
    ...
    return `MCP server '${(0, mcpLogging_1.getMcpServerExternalName)(server.name)}'`;
}
class ServerWorker {
    server;
    connectTimeoutMs;
...
class MCPServers {
    ...
    async reconnect(options = {}) {
        ...
        const serversToCleanup = failedOnly
            ...
            : [...this.allServers];
        logger.debug(`Reconnecting MCP servers (failedOnly=${failedOnly}) with ${serversToCleanup.length} target(s).`);
        if (!failedOnly) {
    ...
    recordFailure(server, error, phase) {
        (0, logger_1.logToolActionError)(logger, `Failed to ${phase} ${getMcpServerLogLabel(server)}:`, error);
        this.storeFailure(server, error);
    }
    ...
    async closeServer(server) {
        try {
            ...
            if (isAbortError(err)) {
                ...
                (0, logger_1.logToolActionDebug)(logger, `Close cancelled for ${getMcpServerLogLabel(server)}:`, err);
                this.errorsByServer.set(server, err);
                return false;
            }
            (0, logger_1.logToolActionError)(logger, `Failed to close ${getMcpServerLogLabel(server)}:`, err);
            this.errorsByServer.set(server, err);
            return false;
        }
    }
    ...
    async connectAllParallel(servers) {
        ...
        if (this.strict) {
            const firstFailure = servers.find((server) => this.failedServerSet.has(server) &&
            ...
            if (firstFailure) {
                ...
                throw new Error(`Failed to connect MCP server '${(0, mcpLogging_1.getMcpServerExternalName)(firstFailure.name)}'.`);
            }
        }
    }
    getWorker(server) {
...
function createTimeoutError(action, server, timeoutMs) {
    ...
    const error = new Error(`MCP server ${action} timed out after ${timeoutMs}ms for '${(0, mcpLogging_1.getMcpServerExternalName)(server.name)}'.`);
    error.name = 'TimeoutError';
    return error;
}
function createClosedError(server) {
    const error = new Error(`MCP server '${(0, mcpLogging_1.getMcpServerExternalName)(server.name)}' is closed.`);
    error.name = 'ClosedError';
    return error;
}
function createClosingError(server) {
    const error = new Error(`MCP server '${(0, mcpLogging_1.getMcpServerExternalName)(server.name)}' is closing.`);
