...
exports.isSandboxPathNotFoundError = isSandboxPathNotFoundError;
exports.probeSandboxPathExists = probeSandboxPathExists;
...
`.trim();
function isSandboxPathNotFoundError(error) {
    if (typeof error !== 'object' || error === null) {
        return false;
    }
    try {
        if (error instanceof errors_1.SandboxWorkspaceReadNotFoundError) {
            return true;
        }
...
async function probeSandboxPathExists(options) {
    const quotedPath = (0, shell_1.shellQuote)(options.path);
...
function createPathProbeError(options, result) {
    ...
    return new errors_1.SandboxWorkspaceArchiveReadError(`Failed to determine whether sandbox path exists: ${options.path}${suffix}`, {
        path: options.path,
...
//# sourceMappingURL=pathProbe.js.map