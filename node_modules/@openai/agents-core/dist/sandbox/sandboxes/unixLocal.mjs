...
import { normalizeSandboxClientCreateArgs, validateSandboxArchiveLimits, } from "../client.mjs";
import { normalizeExposedPort, recordExposedPortEndpoint, } from "../session.mjs";
import { normalizeRelativePath } from "../manifest.mjs";
import { SandboxConfigurationError, SandboxUnsupportedFeatureError, } from "../errors.mjs";
...
import { assertHostPathGrantsRebound, mergeManifestEntryDelta, mergeManifestDelta, sanitizeEnvironmentForPersistence, serializeManifest, } from "./shared/manifestPersistence.mjs";
...
export class UnixLocalSandboxSession {
    ...
    async hydrateWorkspace(data, options = {}) {
        await restoreWorkspaceArchive(data, this.state.workspaceRootPath, {
            archiveLimits: options.archiveLimits === undefined
                ? this.archiveLimits
    ...
    async restoreIfNeeded(state, archiveLimits) {
        if (await pathExists(state.workspaceRootPath)) {
            ...
            if (await localSnapshotIsRestorable(state)) {
                return await restoreSnapshotAndMounts(state, state.workspaceRootPath, archiveLimits);
            }
            return state;
        }
        if (!(await localSnapshotIsRestorable(state))) {
...
async function restoreSnapshotAndMounts(state, workspaceRootPath, archiveLimits) {
    const restoredState = await restoreLocalSnapshotToWorkspace(state, workspaceRootPath, { archiveLimits });
...
function assertUnixLocalHostPathGrantsUnsupported(manifest) {
    const grant = manifest.extraPathGrants.find(({ hostPath }) => hostPath !== undefined);
    if (!grant) {
        return;
    }
    throw new SandboxUnsupportedFeatureError(`UnixLocalSandboxClient does not support path grant hostPath for "${grant.path}". Omit hostPath when the host and sandbox paths are the same, or use DockerSandboxClient when they differ.`, {
        provider: 'UnixLocalSandboxClient',
