...
import { SandboxConfigurationError, SandboxMountError, SandboxUnsupportedFeatureError, SandboxWorkspaceReadNotFoundError, } from "../errors.mjs";
...
import { isEnvValueReference, Manifest } from "../manifest.mjs";
...
import { assertHostPathGrantsRebound, mergeManifestEntryDelta, mergeManifestDelta, sanitizeEnvironmentForPersistence, serializeHostPathGrantRedactionMetadata, serializeManifest, } from "./shared/manifestPersistence.mjs";
...
import { probeSandboxPathExists } from "../shared/pathProbe.mjs";
...
export class DockerSandboxClient {
    ...
    async canReusePreservedOwnedSession(state, options = {}) {
        ...
        if (state.image !== (resolvedOptions.image ?? DEFAULT_DOCKER_IMAGE) ||
            !dockerExposedPortSetsEqual(state.configuredExposedPorts ?? [], normalizeExposedPorts(resolvedOptions.exposedPorts))) {
            return false;
        }
    ...
    async restoreIfNeeded(state, archiveLimits) {
        ...
        if (isRunStateSessionState(state)) {
            ...
            const trustedState = {
                ...
                configuredExposedPorts: normalizeExposedPorts(resolvedOptions.exposedPorts),
                dockerVolumeNames: [],
        ...
        if (containerRunning &&
            !dockerContainerIdentityMatches(state.sessionIdentity, await inspectDockerContainerLabels(state.containerId))) {
            throw new UserError('Docker sandbox container identity could not be verified. Refusing to resume or replace the running container.');
        }
        if (!(await localSnapshotIsRestorable(state))) {
            throw new UserError('Docker sandbox resources are unavailable and no local snapshot could be restored.');
