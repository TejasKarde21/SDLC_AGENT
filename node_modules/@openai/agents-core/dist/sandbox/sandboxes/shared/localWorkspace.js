...
async function applyOwnershipRecursive(targetPath, uid, gid) {
    const info = await (0, promises_1.lstat)(targetPath).catch((error) => {
...
async function pathExists(path) {
    try {
        ...
        let info;
        try {
            info = await (0, promises_1.lstat)(path);
        }
        catch (probeError) {
            if ((0, pathProbe_1.isSandboxPathNotFoundError)(probeError)) {
                return false;
            }
            throw probeError;
        }
        if (info.isSymbolicLink()) {
            try {
                ...
                if ((0, pathProbe_1.isSandboxPathNotFoundError)(retryError)) {
                    return false;
                }
                throw retryError;
            }
...
function resolveLocalSourcePath(entryType, sourcePath, options) {
    ...
    if ((0, hostPath_1.isHostPathWithinRoot)(base, resolvedSourcePath) ||
        (options.localSourceGrants ?? []).some((grant) => (0, hostPath_1.isHostPathWithinRoot)((0, node_path_1.resolve)((0, hostPath_1.sandboxPathGrantHostPath)(grant)), resolvedSourcePath))) {
