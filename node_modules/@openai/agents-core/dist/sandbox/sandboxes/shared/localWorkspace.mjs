...
import { isHostPathWithinRoot, isHostPathStrictlyWithinRoot, relativeHostPathEscapesRoot, sandboxPathGrantHostPath, } from "../../shared/hostPath.mjs";
import { isSandboxPathNotFoundError } from "../../shared/pathProbe.mjs";
...
function resolveLocalSourcePath(entryType, sourcePath, options) {
    ...
    if (isHostPathWithinRoot(base, resolvedSourcePath) ||
        (options.localSourceGrants ?? []).some((grant) => isHostPathWithinRoot(resolve(sandboxPathGrantHostPath(grant)), resolvedSourcePath))) {
