...
import { dir } from "../entries/index.mjs";
...
import { renderMemoryReadPrompt } from "../memory/prompts.mjs";
import { joinRelativePaths, SandboxMemoryStorage, } from "../memory/storage.mjs";
...
class MemoryCapability extends Capability {
    ...
    constructor(args = {}) {
        ...
        if (this.read === null && this.generate === null) {
            throw new UserError('Memory requires at least one of `read` or `generate`.');
        }
    }
    ...
    async readMemorySummary(session) {
        const path = joinRelativePaths(this.layout.memoriesDir, this.layout.summaryFile);
        if (session.pathExists && !(await session.pathExists(path, this._runAs))) {
            return await this.readStoredText(path);
        }
        if (session.readFile) {
            try {
                const payload = await withSandboxSpan('sandbox.memory.read_summary', {
                    path,
                    run_as: this._runAs,
        ...
        throw new UserError('Memory sandbox sessions must provide readFile() or execCommand().');
    }
...
export function memory(args = {}) {
    return new MemoryCapability(args);
}
export function isMemoryCapability(capability) {
...
function normalizeGenerateConfig(config) {
    ...
    if (!Number.isInteger(maxRawMemoriesForConsolidation) ||
        ...
        throw new UserError('MemoryGenerateConfig.maxRawMemoriesForConsolidation must be an integer between 1 and 4096.');
    }
...
function normalizeRequiredRelativePath(name, value) {
    ...
    try {
        ...
        throw new UserError(`${name} must be relative to the sandbox workspace root and must not escape root, got: ${value}`);
    }
    if (!normalized) {
...
async function readMemorySummaryWithShell(args) {
    ...
    const output = await withSandboxSpan('sandbox.memory.read_summary', {
        path: args.path,
        run_as: args.runAs,
...
function rejectKnownSnakeCaseConfigKeys(value, keys, source) {
    ...
    throw new UserError(`Use camelCase config keys in ${source}; snake_case key "${found}" is not supported.`);
}
