...
const prompts_1 = require("../runtime/prompts.js");
const base_1 = require("./base.js");
class SkillsCapability extends base_1.Capability {
    type = 'skills';
    skills;
    from;
    lazyFrom;
    ...
    constructor(args) {
        ...
        this.skillsPath = (0, manifest_1.normalizeRelativePath)(args.skillsPath ?? '.agents');
    ...
    tools() {
        ...
        if (!session.pathExists || !session.materializeEntry) {
            throw new errors_1.SandboxSkillsConfigError('Skills sandbox sessions must provide pathExists() and materializeEntry().');
        }
        return [
            (0, tool_1.tool)({
                name: 'load_skill',
                ...
                execute: async ({ skill_name, }) => {
                    ...
                    const relativeSkillPath = (0, manifest_1.normalizeRelativePath)(match.path ?? match.name);
    ...
    processManifest(manifest) {
        const existingPaths = new Set(Object.keys(manifest.entries).map((path) => (0, manifest_1.normalizeRelativePath)(path)));
        if (this.lazyFrom) {
            const overlaps = [...existingPaths]
                .filter((path) => pathsOverlap(path, this.skillsPath))
                .sort();
            if (overlaps.length > 0) {
                throw new errors_1.SandboxSkillsConfigError(`skills lazyFrom path overlaps existing manifest entries: ${overlaps.join(', ')}`);
            }
            return manifest;
        }
        if (this.from) {
            ...
            if (overlap) {
                throw new errors_1.SandboxSkillsConfigError(`skills path overlaps existing manifest entries: ${overlap}`);
            }
        ...
        for (const skill of this.skills) {
            ...
            if (overlap) {
                throw new errors_1.SandboxSkillsConfigError(`skill path overlaps existing manifest entries: ${skillPath}`);
            }
    ...
    async resolveRuntimeMetadata(_manifest) {
        ...
        try {
            ...
            if ((0, pathProbe_1.isSandboxPathNotFoundError)(error)) {
                return [];
            }
            throw error;
        }
...
const SKILLS_SECTION_INTRO = (0, prompts_1.prompt) `
A skill is a set of local instructions to follow that is stored in a \`SKILL.md\` file. Below is the list of skills that can be used. Each entry includes a name, description, and file path so you can open the source for full instructions when using a specific skill.
`;
const HOW_TO_USE_SKILLS_SECTION = (0, prompts_1.prompt) `
### How to use skills
...
`;
const HOW_TO_USE_LAZY_SKILLS_SECTION = (0, prompts_1.prompt) `
### How to use skills
...
function validateSkillsCapability(capability) {
    ...
    if (configuredSources === 0) {
        throw new errors_1.SandboxSkillsConfigError('skills capability requires `skills`, `from`, or `lazyFrom`.');
    }
    if (configuredSources > 1) {
        throw new errors_1.SandboxSkillsConfigError('skills capability accepts only one of `skills`, `from`, or `lazyFrom`.');
    }
    if (capability.from && !(0, entries_1.isDirectoryLikeEntry)(capability.from)) {
        throw new errors_1.SandboxSkillsConfigError('skills from must be a directory-like entry such as dir, git_repo, or mount.');
    }
    const seenSkillNames = new Set();
    for (const skill of capability.skills) {
        const normalizedName = (0, manifest_1.normalizeRelativePath)(skill.name);
        if (seenSkillNames.has(normalizedName)) {
            throw new errors_1.SandboxSkillsConfigError(`duplicate skill name: ${skill.name}`);
        }
...
function resolveSkillsMetadata(capability, runtimeMetadata = [], manifest) {
    ...
    return configuredIndex
        .map((skill) => ({
        ...skill,
        path: joinRelativePaths(capability.skillsPath, skill.path ?? (0, manifest_1.normalizeRelativePath)(skill.name)),
    }))
        .sort((left, right) => left.name.localeCompare(right.name));
}
...
function deriveIndexFromDirEntry(entry) {
    if (!entry || !(0, entries_1.isDir)(entry) || !entry.children) {
        return [];
    }
...
function resolveLazySkillMatch(capability, skillName, manifest) {
    ...
    const matches = index.filter((skill) => {
        const relativePath = (0, manifest_1.normalizeRelativePath)(skill.path ?? skill.name);
    ...
    if (matches.length === 0) {
        throw new errors_1.SandboxSkillsConfigError(`lazy skill not found: ${skillName}`);
    }
    if (matches.length > 1) {
        throw new errors_1.SandboxSkillsConfigError(`lazy skill name is ambiguous: ${skillName}`);
    }
    return matches[0];
}
function joinRelativePaths(left, right) {
    const normalizedLeft = (0, manifest_1.normalizeRelativePath)(left);
    const normalizedRight = (0, manifest_1.normalizeRelativePath)(right);
    if (!normalizedLeft) {
...
function resolveLazySkillEntry(source, relativeSkillPath) {
    if (source.type === 'local_dir') {
        if (!source.src) {
            throw new errors_1.SandboxSkillsConfigError('Lazy skill local_dir sources require a concrete src value.');
        }
        return {
            ...source,
            type: 'local_dir',
    ...
    if (source.type === 'git_repo') {
        const baseSubpath = source.subpath
            ? (0, manifest_1.normalizeRelativePath)(source.subpath)
            : '';
        return {
            ...source,
            type: 'git_repo',
    ...
    if (source.type === 'dir') {
        ...
        if (!child) {
            throw new errors_1.SandboxSkillsConfigError(`lazy skill source path not found: ${relativeSkillPath}`);
        }
        return child;
    }
    throw new errors_1.SandboxSkillsConfigError(`Lazy skill source type is not supported yet: ${source.type}`);
}
function readDirChild(source, relativeSkillPath) {
    let current = source;
    for (const segment of (0, manifest_1.normalizeRelativePath)(relativeSkillPath).split('/')) {
        if (!segment) {
            continue;
        }
        if (!current || !(0, entries_1.isDir)(current) || !current.children) {
            return undefined;
        }
