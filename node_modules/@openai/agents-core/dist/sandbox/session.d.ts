...
export declare const SANDBOX_SESSION_STATE_VERSION: 2;
export declare const SUPPORTED_SANDBOX_SESSION_STATE_VERSIONS: readonly [1, 2];
export type SandboxSessionStateVersion = (typeof SUPPORTED_SANDBOX_SESSION_STATE_VERSIONS)[number];
...
export declare function normalizeExposedPort(port: number): number;
export declare function exposedPortStateKey(port: number): string;
export declare function recordExposedPortEndpoint(state: SandboxSessionState, endpoint: ExposedPortEndpoint, keyPort?: number): ExposedPortEndpoint;
export declare function getRecordedExposedPortEndpoint(state: SandboxSessionState, port: number): ExposedPortEndpoint | undefined;
export declare function urlForExposedPort(endpoint: ExposedPortEndpoint, scheme: ExposedPortScheme): string;
export interface SandboxSession<TState extends SandboxSessionState = SandboxSessionState> {
    ...
    hydrateWorkspace?(data: WorkspaceArchiveData, options?: WorkspaceArchiveOptions): Promise<void>;
    setArchiveLimits?(limits?: SandboxArchiveLimits | null): void;
    resolveExposedPort?(port: number): Promise<ExposedPortEndpoint>;
    supportsPty?(): boolean;
    close?(): Promise<void>;
}
export type SandboxSessionLike<TState extends SandboxSessionState = SandboxSessionState> = SandboxSession<TState>;
