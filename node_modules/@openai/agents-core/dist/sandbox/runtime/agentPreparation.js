...
function prepareSandboxAgent({ agent, session, capabilities, runConfigModel, processManifest = true, tracingParent, }) {
    ...
    const canEditorAccessPath = boundCapabilityTypes.has('filesystem')
        ? resolveEditorPathAccess(session, (0, runAsManifest_1.sandboxRunAsName)(agent.runAs))
        : undefined;
    ...
    const prepared = agent.clone({
        ...
        instructions: async (runContext, preparedAgent) => {
            ...
            const remoteMountPolicy = (0, prompts_1.renderRemoteMountPolicyInstructions)(runtimeManifest, canEditorAccessPath);
...
function resolveEditorPathAccess(session, runAs) {
