...
import { getToolCallParentSpanFromDetails, getAgentToolParentRunConfigFromDetails, getInheritedAgentToolRunConfig, mergeAgentToolRunConfig, } from "./agentToolRunConfig.mjs";
...
import { getToolUsageRecorder, setRunnerParentUsageRecorder, } from "./runner/usageTracking.mjs";
import { setRunnerInvocationSpanParent } from "./runner/invocationContext.mjs";
import { hasDefinitelyDifferentOutputTypes } from "./agentOutputTypeWarning.mjs";
...
export class Agent extends AgentHooks {
    ...
    constructor(config) {
        ...
        if (config.handoffOutputTypeWarningEnabled === undefined ||
            ...
            if (this.handoffs && this.outputType) {
                const outputTypes = [this.outputType];
                ...
                if (logger.dontLogModelData) {
                    if (hasDefinitelyDifferentOutputTypes(outputTypes)) {
                        logger.warn('[Agent] Warning: Handoff agents have different output types. Output type details are redacted. You can make it type-safe by using Agent.create({ ... }) method instead.');
                    }
                }
                else {
                    const serializedOutputTypes = outputTypes.map((type) => JSON.stringify(type));
                    if (new Set(serializedOutputTypes).size > 1) {
                        logger.warn(`[Agent] Warning: Handoff agents have different output types: ${serializedOutputTypes.join(', ')}. You can make it type-safe by using Agent.create({ ... }) method instead.`);
                    }
                }
            }
        }
    ...
    asTool(options) {
        ...
        const baseTool = tool({
            ...
            execute: async (params, context, details) => {
                ...
                setRunnerInvocationSpanParent(runner, getToolCallParentSpanFromDetails(details));
                ...
                try {
                    ...
                    if (shouldStream) {
                        ...
                        if (streamResult.cancelled) {
                            ...
                            const hasCommittedOutcome = (currentStep?.type === 'next_step_final_output' &&
     ...
     */
    async getMcpTools(runContext, tracingParent) {
     ...
     */
    async getAllTools(runContext, tracingParent) {
