...
function getApprovalToolNameCandidates(toolName, includeFunctionAliases) {
    ...
    if (includeFunctionAliases) {
        const bareFunctionKey = toolName.startsWith('[')
            ? undefined
            : (0, toolIdentity_1.getFunctionToolLookupKey)(toolName);
...
class RunContext {
     ...
     * @internal
     */
    _rebuildFunctionApprovals(approvals, agentsByIdentity) {
     ...
     * @internal
     */
    _rebuildLegacyFunctionApprovals(approvals) {
     ...
     * @internal
     */
    _mergeLegacyFunctionApprovals(approvals) {
        for (const [toolName, incoming] of Object.entries(approvals)) {
     ...
     * @internal
     */
    _retainLegacyFunctionApprovals(toolNames) {
     ...
     * @internal
     */
    _removeMigratedFunctionApprovalsFromAggregate(functionToolNames, exactNonFunctionToolNames) {
     ...
     * @internal
     */
    _mergeFunctionApprovals(approvals, agentsByIdentity) {
     ...
     * @internal
     */
    _validateFunctionApprovalOwners(approvals, agentsByIdentity) {
        const seenAgentIdentities = new Set();
        for (const { agentIdentity } of approvals) {
            if (seenAgentIdentities.has(agentIdentity) ||
                !agentsByIdentity.has(agentIdentity)) {
                throw new errors_1.UserError('RunState contains invalid function approval ownership for the current agent graph.');
            }
     ...
     * @internal
     */
    _mergeApprovalStatePreservingExactKeys(source, exactToolNames) {
        for (const [toolName, incoming] of source.#approvals) {
     ...
     * @internal
     */
    _migrateToolApproval(agent, legacyToolName, canonicalToolName, callIds, migratePermanentDecision, preserveMovedCallDecisions = false) {
        if (legacyToolName === canonicalToolName) {
            return;
        }
        const legacy = this.#functionApprovalState.legacyApprovals.get(legacyToolName);
        if (!legacy) {
            return;
        }
        const callIdSet = new Set(callIds);
        const movedApproved = legacy.approved === true
            ...
            : Array.isArray(legacy.approved)
                ...
                : [];
        const movedRejected = legacy.rejected === true
            ? migratePermanentDecision
                ? true
                : [...callIdSet]
        ...
        let movedMessages = legacy.messages
            ? Object.fromEntries(Object.entries(legacy.messages).filter(([callId]) => callIdSet.has(callId)))
            : undefined;
        if (legacy.rejected === true &&
            !migratePermanentDecision &&
        ...
        const hasMovedDecision = moved.approved === true ||
            moved.rejected === true ||
        ...
        const remainingApproved = legacy.approved === true
            ? migratePermanentDecision
                ? []
                : true
            : Array.isArray(legacy.approved)
                ? legacy.approved.filter((callId) => preserveMovedCallDecisions || !callIdSet.has(callId))
                : [];
        const remainingRejected = legacy.rejected === true
            ? migratePermanentDecision
                ? []
                : true
            : Array.isArray(legacy.rejected)
                ? legacy.rejected.filter((callId) => preserveMovedCallDecisions || !callIdSet.has(callId))
                : [];
        const remainingMessages = legacy.messages
            ? Object.fromEntries(Object.entries(legacy.messages).filter(([callId]) => preserveMovedCallDecisions || !callIdSet.has(callId)))
            : undefined;
        const remaining = {
        ...
        const hasRemainingDecision = remaining.approved === true ||
        ...
        if (hasRemainingDecision) {
            this.#functionApprovalState.legacyApprovals.set(legacyToolName, remaining);
        }
        else {
     ...
     */
    getRejectionMessage(toolName, callId, options = {}) {
        const functionTool = options.functionTool ?? true;
        const includeFunctionState = functionTool ||
            (0, toolIdentity_1.getFunctionToolLegacyStateKeyFromStateKey)(toolName) !== undefined;
     ...
     * @internal
     */
    _getFunctionRejectionMessage(toolName, callId, agent) {
        const scopedEntries = this.#getFunctionApprovalEntries(toolName, false, agent);
        const scopedDecision = this.#resolveApprovalEntries(scopedEntries, callId);
        const scopedMessage = this.#getRejectionMessageFromEntries(scopedEntries, callId);
        ...
        if (scopedDecision === undefined &&
            ...
            return this.#getRejectionMessageFromEntries(this.#getLegacyFunctionApprovalEntries(toolName), callId);
        }
        return undefined;
    }
    /**
     ...
     * @internal
     */
    _toJSONForRunState(agentIdentityKeys) {
        return this.#serialize(agentIdentityKeys);
    }
    #serialize(agentIdentityKeys) {
        ...
        if (!agentIdentityKeys) {
            for (const [toolName, incoming] of this.#functionApprovalState
                ...
                approvals.set(toolName, current ? mergeApprovalRecords(current, incoming) : incoming);
            }
            for (const approvalsByTool of this.#functionApprovalState.approvalsByAgent.values()) {
                for (const [toolName, incoming] of approvalsByTool) {
                    const publicToolName = (0, toolIdentity_1.getFunctionToolLegacyStateKeyFromStateKey)(toolName) ?? toolName;
                    const current = approvals.get(publicToolName);
                    approvals.set(publicToolName, current ? mergeApprovalRecords(current, incoming) : incoming);
                }
            }
        }
        const json = {
        ...
        if (agentIdentityKeys) {
            ...
            if (this.#functionApprovalState.legacyApprovals.size > 0) {
                json.legacyFunctionApprovals = Object.fromEntries(this.#functionApprovalState.legacyApprovals);
            }
        }
    ...
    isToolApproved(approval) {
        ...
        if (agent) {
            const scopedDecision = this.#resolveApprovalEntries(this.#getFunctionApprovalEntries(toolName, functionTool, agent), callId);
            if (scopedDecision !== undefined) {
            ...
            return this.#resolveApprovalEntries(this.#getLegacyFunctionApprovalEntries(toolName), callId);
        }
        const includeFunctionState = functionTool ||
            (0, toolIdentity_1.getFunctionToolLegacyStateKeyFromStateKey)(toolName) !== undefined;
    ...
    #resolveApprovalEntries(approvalEntries, callId) {
    ...
    #getRejectionMessageFromEntries(approvalEntries, callId) {
    ...
    approveTool(approvalItem, { alwaysApprove = false } = {}) {
        ...
        const approvals = approvalItem.rawItem.type === 'function_call'
            ? this.#getFunctionApprovalMap(approvalItem.agent)
            : this.#approvals;
        const approvalKey = this.#getApprovalStorageKey(toolName, approvalItem.rawItem.type === 'function_call', approvals);
        if (alwaysApprove) {
    ...
    #getApprovalEntries(toolName, includeFunctionAliases) {
    ...
    #getFunctionApprovalEntries(toolName, includeFunctionAliases, agent) {
        const candidates = getApprovalToolNameCandidates(toolName, includeFunctionAliases);
        ...
        return approvalMaps.flatMap((approvalsByTool) => approvalsByTool
            ? candidates
                .map((candidate) => approvalsByTool.get(candidate))
                .filter((approval) => approval !== undefined)
            : []);
    }
    #getLegacyFunctionApprovalEntries(toolName) {
        const legacyToolName = (0, toolIdentity_1.getFunctionToolLegacyStateKeyFromStateKey)(toolName) ?? toolName;
        return getApprovalToolNameCandidates(legacyToolName, true)
            .map((candidate) => this.#functionApprovalState.legacyApprovals.get(candidate))
            .filter((approval) => approval !== undefined);
    }
    #getFunctionApprovalMap(agent) {
    ...
    #getApprovalItemToolName(approvalItem) {
        const fallbackName = approvalItem.toolName ?? approvalItem.rawItem.name;
        if (approvalItem.rawItem.type !== 'function_call') {
            return fallbackName;
        }
        return (approvalItem.functionToolStateKey ??
            (0, toolIdentity_1.getFunctionToolStateKeyForCall)(approvalItem.rawItem, fallbackName) ??
            fallbackName);
    }
    #getApprovalStorageKey(toolName, includeFunctionAliases, approvals = this.#approvals) {
        return (getApprovalToolNameCandidates(toolName, includeFunctionAliases).find((candidate) => approvals.has(candidate)) ?? toolName);
    }
    #setFunctionApprovalRecord(agent, toolName, incoming) {
        const approvalsByTool = this.#getFunctionApprovalMap(agent);
        const current = approvalsByTool.get(toolName);
        approvalsByTool.set(toolName, current ? mergeApprovalRecords(current, incoming) : incoming);
    }
    #setLegacyFunctionApprovalRecord(toolName, incoming) {
        const current = this.#functionApprovalState.legacyApprovals.get(toolName);
        this.#functionApprovalState.legacyApprovals.set(toolName, current ? mergeApprovalRecords(current, incoming) : incoming);
    }
