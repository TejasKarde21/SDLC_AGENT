...
export class OpenAIChatCompletionsModel {
    ...
    async getResponse(request) {
        ...
        const { response, rawResponse } = await withGenerationSpan(async (span) => {
    ...
    #handleUnsupportedReasoningSettings(request) {
        ...
        if (!this.#hasWarnedUnsupportedReasoningSettings) {
            logger.warn(`${message} Ignoring unsupported reasoning settings; ` +
                'enable strict feature validation to raise an error instead.');
        ...
        if (request.tools) {
            for (const tool of request.tools) {
                if ((tool.type === 'function' ||
                    ...
                    throw new UserError('Tools callable from Programmatic Tool Calling are only supported with the Responses API.');
                }
                if (tool.type === 'function' && tool.outputSchema) {
                    throw new UserError('Function tool outputSchema is only supported with the Responses API.');
                }
                if (tool.type === 'hosted_tool' &&
                    ...
                    throw new UserError('Programmatic Tool Calling is only supported with the Responses API.');
                }
                if (tool.type === 'function') {
