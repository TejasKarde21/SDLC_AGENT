...
import { assertValidCompactionItems, formatInlineData, getInlineMediaType, } from '@openai/agents-core/utils/internal';
...
function convertLegacyToolOutputContent(output) {
    ...
    if (output.type === 'image') {
        ...
        const topLevelInlineMediaType = getInlineMediaType(output);
        if (typeof output.image === 'string' && output.image.length > 0) {
            structured.image = output.image;
        }
        else if (isRecord(output.image)) {
            const imageObj = output.image;
            const inlineMediaType = getInlineMediaType(imageObj) ?? topLevelInlineMediaType;
...
function getMessageItem(item) {
    ...
    if (item.role === 'assistant') {
        ...
        if (typeof phase !== 'undefined' &&
            phase !== 'commentary' &&
            phase !== 'final_answer') {
            throw new UserError(`Invalid assistant message phase: ${JSON.stringify(phase)}. Expected "commentary" or "final_answer".`);
        }
