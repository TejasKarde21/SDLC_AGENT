import { Usage, UserError, } from '@openai/agents-core';
import OpenAI from 'openai';
import { ResponsesWS } from 'openai/resources/beta/responses/ws';
import { HEADERS } from "../../defaults.mjs";
import { OpenAIResponsesModel, convertToOutputItem, } from "../../openaiResponsesModel.mjs";
import { applyHeadersToAccumulator, createHeaderAccumulator, headerAccumulatorToRecord, mergeQueryParamsIntoURL, } from "../../responsesTransportUtils.mjs";
import { withAbortSignal, withTimeout, } from "../../responsesWebSocketConnection.mjs";
import { searchParamsToAuthHeaderQuery, toRequestUsageEntry, } from "../../responsesUtils.mjs";
...
function mergeResponseUsages(usages) {
    return new Usage({
        ...
        requestUsageEntries: usages.map((usage) => toRequestUsageEntry(usage, 'responses.create')),
    });
}
...
function validateConfig(config) {
    ...
    if (typeof maxConcurrentSubagents !== 'undefined' &&
        ...
        throw new UserError('HostedMultiAgentConfig.maxConcurrentSubagents must be a positive integer.');
    }
...
export class OpenAIHostedMultiAgentModel extends OpenAIResponsesModel {
    ...
    _getResponsesCreateRequestOverrides(request, requestData) {
        if (request.handoffs.length > 0) {
            throw new UserError('OpenAIHostedMultiAgentModel does not support SDK handoffs. Remove the Agent handoffs or use OpenAIResponsesModel.');
        }
        if (requestData.reasoning?.summary != null) {
            throw new UserError('OpenAIHostedMultiAgentModel does not support reasoning.summary. Remove the reasoning summary setting.');
        }
        if (requestData.max_tool_calls != null) {
            throw new UserError('OpenAIHostedMultiAgentModel does not support max_tool_calls. Remove that provider setting.');
        }
        return {
            multi_agent: {
                enabled: true,
    ...
    _convertResponseOutputItems(items) {
        ...
        return convertToOutputItem(stableItems);
    }
    ...
    async *#runWebSocketTurn(request) {
        if (this.#requestInProgress) {
            throw new UserError('OpenAIHostedMultiAgentModel does not support concurrent requests on one model instance.');
        }
            ...
            if (this.#activeResponse) {
                ...
                if (activeResponse.pendingContinuationFrame) {
                    ...
                    if (missingCallIds.length > 0) {
                        throw new UserError(`The hosted response is waiting for local function outputs for: ${missingCallIds.join(', ')}. Resume it with the same model instance and RunState.`);
                    }
                    if (activeResponse.terminalEvent) {
                        ...
                        if (outputs.length === 0 || !activeResponse.responseId) {
                            throw new UserError('The hosted response is waiting for local function outputs. Resume it with the same model instance and RunState.');
                        }
                        pendingInjections = 1;
    ...
    async #ensureWebSocket(transportOptions, activeResponse, signal, requestTimeoutDeadline) {
        ...
        if (activeResponse) {
            if (transportOverridesKey !== this.#webSocketTransportOverridesKey) {
                throw new UserError('An active hosted response must be resumed with the same WebSocket transport headers and query.');
            }
            if (this.#webSocket) {
    ...
    async #prepareWebSocketCreationOptions(transportOptions, signal, requestTimeoutDeadline) {
        ...
        const authHeaderQuery = searchParamsToAuthHeaderQuery(websocketURL.searchParams);
        ...
        applyHeadersToAccumulator(headerAccumulator, client._options?.defaultHeaders);
        applyHeadersToAccumulator(headerAccumulator, HEADERS);
        applyHeadersToAccumulator(headerAccumulator, transportOptions.extraHeaders, { allowBlockedOverride: true });
        applyHeadersToAccumulator(headerAccumulator, { 'OpenAI-Beta': MULTI_AGENT_BETA }, { allowBlockedOverride: true });
        return {
    ...
    async #nextWebSocketMessage(signal, requestTimeoutDeadline) {
        ...
        const result = await withAbortSignal(withTimeout(iterator.next(), frameReadTimeout.timeoutMs, frameReadTimeout.errorMessage), signal);
