        ...
        if (isMessageItem(item)) {
            ...
            if (role === 'assistant') {
                ...
                if (typeof phase !== 'undefined' && phase !== null) {
                    ...
                    const message = 'Assistant message phase is not supported by the Chat Completions API. Use the Responses API to preserve assistant message phases.';
                    if (options.strictFeatureValidation) {
                        throw new agents_core_1.UserError(message);
                    }
                    logger_1.default.warn(`${message} The phase will be dropped.`);
                }
                const assistant = {
                    role: 'assistant',
            ...
            if (item.caller?.type === 'program') {
                throw new agents_core_1.UserError('Programmatic Tool Calling history is not supported for chat completions. Please use the Responses API.');
            }
