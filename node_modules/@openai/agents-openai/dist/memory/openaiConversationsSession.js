...
async function startOpenAIConversationsSession(client) {
    const resolvedClient = client ? client : resolveClient({});
...
class OpenAIConversationsSession {
    ...
    #client;
    #conversationId;
    #conversationIdOperation = Promise.resolve();
    #activeItemOperations = new Set();
    constructor(options = {}) {
    ...
    async getItems(limit) {
        return this.#runItemOperation((conversationId) => this.#getItems(conversationId, limit));
    }
    async #getItems(conversationId, limit) {
    ...
    prepareHistoryItemsForPersistenceComparison(items) {
        return normalizeItemsForConversationPersistence(items);
    }
    ...
    async #runItemOperation(operation) {
    ...
    #runConversationIdOperation(operation) {
        const result = this.#conversationIdOperation.then(operation);
        this.#conversationIdOperation = result.then(() => undefined, () => undefined);
        return result;
    }
}
...
//  Internals
// --------------------------------------------------------------
function normalizeItemsForConversationPersistence(items) {
    const normalizedItems = stripProviderModelForConversationPersistence(items);
    return stripConversationPersistenceMetadata((0, openaiResponsesModel_1.getInputItems)(normalizedItems));
}
...
function normalizeSystemMessageContent(content) {
    ...
    return content
        .map((item) => item.type === 'input_text' || item.type === 'text' ? item.text : null)
...
function stripAssistantReplayMetadata(item) {
    ...
    const { id: _id, providerData, provider_data: _provider_data, ...rest } = record;
    if (typeof rest.phase === 'undefined' &&
        providerData &&
        typeof providerData === 'object' &&
        !Array.isArray(providerData) &&
        typeof providerData.phase !== 'undefined') {
        rest.phase = providerData.phase;
    }
    return rest;
}
